...
class Transformer(nn.Module):
    def __init__(self, length, channels, num_layers, d_model, n_token, n_head, dim_feedforward, dropout,
        ...
        # Adapter 설정 - None이면 Adapter 비활성화
        self._use_adapter = adapter_bottleneck_dim is not None and adapter_dropout is not None
        if self._use_adapter:
            print(f"Transformer v3: Adapter ENABLED (bottleneck_dim={adapter_bottleneck_dim})")
        else:
            print("Transformer v3: Adapter DISABLED (Pure Transformer)")

        # Transformer layers
        self._layers = nn.ModuleList()
        for _ in range(self._num_layers):
            # Adapter 비활성화 시 None 전달
            layer_adapter_dim = adapter_bottleneck_dim if self._use_adapter else None
            layer_adapter_dropout = adapter_dropout if self._use_adapter else None
            ...
            layer = TransformerLayer(self._d_model, self._n_token, n_head, dim_feedforward, dropout,
                                     ...
                                     adapter_bottleneck_dim=layer_adapter_dim, # Adapter 관련 파라미터 전달
                                     adapter_dropout=layer_adapter_dropout)
