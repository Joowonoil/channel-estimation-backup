"""
Outdoor Base Model Training Engine
야외 환경(UMa + UMi + RMa) 전용 베이스 모델 학습

이 스크립트는 Cross-Domain Transfer Learning 실험의 일부로,
야외 환경에서만 학습된 베이스 모델을 생성합니다.
"""
...
class TrainingEngine:
    def __init__(self, conf_file='config_v4_outdoor_base.yaml'):
        ...
        self._wandb_proj = self._conf['training'].get('wandb_proj', 'DNN_channel_estimation_Outdoor_Base')
    ...
    def train(self):
        ...
        for it, data in enumerate(self._dataloader):
            ...
            if (it + 1) % self._conf['training']['logging_step'] == 0:
                ...
                print(f"[OUTDOOR BASE] iteration: {it + 1}, ch_nmse: {ch_nmse}, lr: {current_lr}")
    ...
    def save_model(self, file_name):
        ...
        print(f"[OUTDOOR BASE] Model saved to {full_path}")

if __name__ == "__main__":
    print("=" * 60)
    print("Outdoor Base Model Training Start")
    print("=" * 60)
    print("Dataset: UMa_Los, UMa_Nlos, UMi_Los, UMi_Nlos, RMa_Los, RMa_Nlos")
    print("Purpose: Cross-Domain Transfer Learning Base Model")
    print("Target: Outdoor environments (open space, various distances)")
    print("=" * 60)
    ...
    print("=" * 60)
    print("Outdoor Base Model Training Completed!")
    print("Model: Large_estimator_v4_outdoor_base.pt")
    print("Ready for Indoor transfer learning")
    print("=" * 60)
    ...
    try:
        final_model_name = engine._conf['training'].get('saved_model_name', 'Large_estimator_v4_outdoor_base')
