...
class Engine_v4:
    def __init__(self, conf_file, device=None, use_wandb=None, wandb_proj=None):
        ...
        self._wandb_proj = wandb_proj or self._conf['training'].get('wandb_proj', 'DNN_channel_estimation_v4_base')
        if self._use_wandb:
        ...
        # Channel and phase noise estimation network (v4 사용)
    ...
    def _logging(self, it, ch_nmse, ch_est, ch_true):
        ...
        if (it + 1) % self._conf['training']['evaluation_step'] == 0:
            ...
            # v4 베이스 모델 저장 (LoRA 미적용 상태)
            save_name = self._conf['training'].get('saved_model_name', 'Large_estimator_v4_base')
    ...
    def save_model(self, file_name):
        """v4 베이스 모델을 저장 - LoRA 미적용 순수 v4 구조"""
        path = Path(__file__).parents[0].resolve() / 'saved_model'
        path.mkdir(parents=True, exist_ok=True)
        
        # v4 구조 그대로 저장 (LoRA 미적용)
        torch.save(self._estimator, path / (file_name + '.pt'))
        print(f"v4 base model saved to {path / (file_name + '.pt')}")
    ...
    def load_model(self, file_name):
        ...
        self.set_optimizer()
        print(f"v4 base model loaded from {path / (file_name + '.pt')}")
...
if __name__ == "__main__":
    ...
    # Training v4 base model - 모든 설정을 config_v4.yaml에서 읽음
    conf_file = 'config_v4.yaml'  # v4 기본 config 사용
    engine = Engine_v4(conf_file)  # 설정 파일에서 모든 파라미터 읽음
    ...
    # 베이스 모델 훈련 시작
    print("Starting v4 base model training...")
    ...
    # 최종 모델 저장
    final_model_name = engine._conf['training'].get('saved_model_name', 'Large_estimator_v4_base')
    engine.save_model(f"{final_model_name}_final")
    print(f"v4 base model training completed and saved as {final_model_name}_final.pt")
