...
if __name__ == "__main__": # Urban to Rural 전이학습 실행
    #torch.autograd.set_detect_anomaly(True) # 자동 미분 이상 감지 활성화 (주석 처리됨)
    
    # Urban to Rural 전용 설정 파일 사용
    conf_file = 'config_transfer_v4_Urban_to_Rural.yaml' # Urban to Rural 설정 파일 사용
    
    print("=" * 60)
    print("Urban to Rural LoRA Transfer Learning Start")
    print("=" * 60)
    print(f"Config file: {conf_file}")
    print(f"Base environment: Urban (UMa + UMi)")
    print(f"Target environment: Rural (RMa)")
    print(f"Transfer method: LoRA (Low-Rank Adaptation)")
    print("=" * 60)
    
    engine = TransferLearningEngine(conf_file) # TransferLearningEngine 객체 생성
    engine.load_model() # Urban 베이스 모델 로드 및 LoRA 적용
    
    print("Urban to Rural transfer learning starting...")
    engine.train() # Rural 데이터로 LoRA 전이학습 수행
    
    print("=" * 60)
    print("Urban to Rural LoRA transfer learning completed!")
    print(f"Saved model: Large_estimator_v4_Urban_to_Rural.pt")
    ...
    try:
        final_model_name = engine._conf['training'].get('saved_model_name', 'Large_estimator_v4_Urban_to_Rural')
