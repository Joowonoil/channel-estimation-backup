...
class TransferLearningEngine: # 전이 학습 엔진 클래스 정의
    ...
    def _logging(self, it, ch_nmse, ch_est, ch_true): # 로깅 메서드
        ...
        if (it + 1) % self._conf['training']['evaluation_step'] == 0: # 현재 이터레이션이 평가 스텝의 배수이면 (평가와 동일한 간격으로 플롯 로깅)
            ...
            # 그래프 생성 비활성화 설정 확인
            if not self._conf['training'].get('disable_plotting', False):
                f = plot_signal(sig_dict, shape=(3, 2)) # 신호 플롯 생성 (3행 2열 형태)
                f.show() # 플롯 표시
                if self._use_wandb: # WandB 사용 설정이 True이면
                    wandb.log({'estimation': wandb.Image(f)}) # 플롯 이미지를 WandB에 로깅
            else:
                print(f"Plotting disabled at iteration {it + 1}") # 플롯 비활성화 메시지
...
if __name__ == "__main__": # Outdoor to Indoor 전이학습 실행
    #torch.autograd.set_detect_anomaly(True) # 자동 미분 이상 감지 활성화 (주석 처리됨)
    
    # Outdoor to Indoor 전용 설정 파일 사용
    conf_file = 'config_transfer_v4_Outdoor_to_Indoor.yaml' # Outdoor to Indoor 설정 파일 사용
    
    print("=" * 60)
    print("Outdoor to Indoor LoRA Transfer Learning Start")
    print("=" * 60)
    print(f"Config file: {conf_file}")
    print(f"Base environment: Outdoor (UMa + UMi + RMa)")
    print(f"Target environment: Indoor (InH + InF)")
    print(f"Transfer method: LoRA (Low-Rank Adaptation)")
    print("=" * 60)
    
    engine = TransferLearningEngine(conf_file) # TransferLearningEngine 객체 생성
    engine.load_model() # Outdoor 베이스 모델 로드 및 LoRA 적용
    
    print("Outdoor to Indoor transfer learning starting...")
    engine.train() # Indoor 데이터로 LoRA 전이학습 수행
    
    print("=" * 60)
    print("Outdoor to Indoor LoRA transfer learning completed!")
    print(f"Saved model: Large_estimator_v4_Outdoor_to_Indoor.pt")
    ...
    try:
        final_model_name = engine._conf['training'].get('saved_model_name', 'Large_estimator_v4_Outdoor_to_Indoor')
