...
class TransferLearningEngine: # 전이 학습 엔진 클래스 정의
    ...
    def save_combined_model_as_pt(self, file_name): # 기본 모델과 LoRA 가중치가 결합된 전체 모델의 state_dict를 .pt 파일로 저장하는 메서드
        ...
        try:
            ...
            original_estimator = Estimator('config_v4.yaml').to(self._device)
            ...
            # v4 구조 그대로도 저장 (simple_model_test.py에서 사용)
            v4_path = path / f"{file_name}_v4.pt"
            torch.save(merged_v4_model, v4_path)
            print(f"v4 structure model saved to {v4_path}")
